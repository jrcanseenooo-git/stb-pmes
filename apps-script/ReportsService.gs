...
const ReportsService = (() => {
  ...
  // Participating-office administrators work from their own assessment
  // spreadsheet. The PMES and STB instrument reports below read the central
  // workbook and therefore must never be offered to, or generated by, an
  // assigned non-STB office account. Their office-scoped analytics annex uses
  // the registered office spreadsheet instead.
  const STB_ONLY_TYPES = [
    'ipcrf-summary',
    'ccef-summary',
  ...
  function _canViewClusterReports(profile) {
    // An office may assign the title "System Administrator" to its own
    // administrator. Office assignment always wins over a job title; otherwise
    // that account could read every participating office's reports.
    if (_isAssignedNonStbOffice(profile)) return false
  ...
  function _isAssignedNonStbOffice(profile) {
    const scope = String(profile.systemScope || '').trim().toUpperCase()
    if (scope === 'OFFICE_ADMIN' || scope === 'OFFICE_FULL_PMES' || scope === 'CLUSTER_PORTAL') return true

    const officeId = String(profile.officeId || profile.officeCode || '').trim().toUpperCase()
    return !!officeId && officeId !== 'STB' && officeId !== 'SOCIAL TECHNOLOGY BUREAU'
  }

  function _allowedReportTypes(profile, canBureau, canCluster, canOffice) {
    if (canOffice && !canCluster && _isAssignedNonStbOffice(profile)) {
      return Object.keys(REPORT_TYPES)
        .filter(type => STB_ONLY_TYPES.indexOf(type) < 0)
    }
    return Object.keys(REPORT_TYPES)
      .filter(type => BUREAU_ONLY_TYPES.indexOf(type) < 0 || canBureau)
  }

  function _officeScopeKeys_(profile) {
    const keys = {}
    const add = (value) => {
      const key = String(value || '').trim().toUpperCase()
      if (key) keys[key] = true
    }
    add(profile.officeId)
    add(profile.officeCode)
  ...
  function _reportBelongsToOffice_(report, profile) {
    const officeId = String(report && report.officeId || '').trim().toUpperCase()
    // Legacy reports without an explicit office scope cannot be assigned safely
    // after the fact, so they remain central-only rather than being guessed.
    return !!officeId && !!_officeScopeKeys_(profile)[officeId]
  }

  function _canonicalOfficeId_(profile) {
    const row = _findOfficeRegistryRow(profile.officeId || profile.officeCode)
    return String((row && row.officeId) || profile.officeId || profile.officeCode || '').trim()
  }
  ...
  function _buildSemestral(filters) {
    ...
    const rows = forms.map(f => {
      ...
      return {
        ...
        ipatDescriptor:       linked.overallScore ? _descriptor(linked) : (linked.descriptor || '')
      }
    })
  ...
  function _descriptor(row) {
    ...
    if (score > 0 && typeof IPATService !== 'undefined') return IPATService.qualitativeDescriptor(score)
    const direct = String(row.descriptor || row.interpretation || '').trim()
    return direct || 'Unscored'
  }
  ...
  function list(params, user) {
    ...
    const allowedTypes = _allowedReportTypes(profile, canBureau, canCluster, canOffice)

    let rows = SpreadsheetService.getAllRows(sheet)
      .sort((a, b) => new Date(b.generatedAt) - new Date(a.generatedAt))

    rows = rows.filter(r => allowedTypes.indexOf(String(r.type || '')) >= 0)

    if (_isAssignedNonStbOffice(profile)) {
      // An office-admin group includes monitoring permissions for its own
      // office. Those permissions must not turn an Office Admin Portal account
      // into a central history reader.
      rows = rows.filter(r => _reportBelongsToOffice_(r, profile))
    } else if (!canBureau && !canCluster) {
  ...
  function generate(body, user) {
    ...
    if (_allowedReportTypes(profile, canBureau, canCluster, canOffice).indexOf(type) < 0) {
      throw HttpError('This report is not available for your assigned office and access level.', 403)
    }
  ...
  function download(id, user) {
    ...
    if (_allowedReportTypes(profile, canBureau, canCluster, canOffice).indexOf(String(row.type || '')) < 0) {
      throw HttpError('You do not have access to this report.', 403)
    }

    if (_isAssignedNonStbOffice(profile)) {
      if (!_reportBelongsToOffice_(row, profile)) {
        throw HttpError('You do not have access to this report.', 403)
      }
    } else if (!canBureau && !canCluster) {
      if (String(row.divisionId || '') !== String(profile.divisionId || '')) {
        throw HttpError('You do not have access to this report.', 403)
      }
    }
  ...
  function options(params, user) {
    ...
    return {
      ...
      types: _allowedReportTypes(profile, canBureau, canCluster, canOffice)
        .map(t => ({
          value: t,
          label: t === 'undersecretary-analytics' && canOffice && !canCluster && _isAssignedNonStbOffice(profile)
            ? 'Office Assessment Analytics'
            : REPORT_TYPES[t]
        })),
      formats: FORMATS,

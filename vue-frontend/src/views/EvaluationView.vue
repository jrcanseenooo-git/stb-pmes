...
// STB stores four divisions as short codes, which have to be spelled out. Every
// other office stores a real name.
const STB_DIVISION_CODES = {
...
// An id is not a label. This used to fall back to rateeDivisionId, so offices
// whose assignment rows carry no division name showed raters
// "OFF-OSAS-IPD-DIV-OFFICE-OF-THE-SPECIAL-ASSISTANT-T..." - truncated, shouted,
// and telling them nothing about the person they are rating. Better to show
// nothing than to show plumbing.
//
// The uppercase was for the four STB codes and made sense there; applied to a
// real division name it just shouts. Codes are spelled out, names are left as
// they were written.
function taskDivisionLabel(task) {
  const raw = String(task?.rateeDivisionName || task?.divisionName || '').trim()
  if (!raw) return ''
  const code = STB_DIVISION_CODES[raw.toLowerCase()]
  if (code) return code
  if (looksLikeOrgId(raw)) return ''
  return raw
}

// Ids generated by the office structure look like
// OFF-<OFFICE>-DIV-<slug>. A name never does.
function looksLikeOrgId(value) {
  return /^OFF-[A-Z0-9-]*-(DIV|SEC)-/i.test(String(value || '').trim())
}

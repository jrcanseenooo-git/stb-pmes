  ...
  <div class="page">
    ...
    <div class="filter-bar">
      <div class="filter-tabs">
        <button v-for="t in statusTabs" :key="t.value" :class="['ftab', activeStatus === t.value && 'active']"
          ...
          {{ t.label }}
          <span v-if="t.value !== 'ALL' && countByStatus(t.value)" class="ftab-badge">{{ countByStatus(t.value)
            }}</span>
        </button>
      </div>
    ...
    <div v-else-if="filteredForms.length === 0" class="empty-state">
      <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
        <rect x="7" y="5" width="34" height="38" rx="4" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1.5" />
        <path d="M15 17h18M15 24h13M15 31h9" stroke="#cbd5e1" stroke-width="2" stroke-linecap="round" />
      </svg>
      <div class="empty-title">{{ activeStatus !== 'ALL' ? `No ${activeStatus.toLowerCase()} forms` : 'No forms yet' }}
      </div>
      <div class="empty-sub">{{ activeStatus !== 'ALL' ? 'Try a different filter.' : 'Create your first IPCRF or CCEF form.' }}</div>
      <button v-if="activeStatus.value === 'All'" class="btn btn-primary btn-sm" @click="showNewFormModal = true">Create New
        Form</button>
    </div>
    ...
    <div v-else class="forms-grid">
      <div v-for="form in filteredForms" :key="form.id" class="form-card" @click="openFormModal(form)">
        <div class="fc-top">
          ...
          <span :class="['status-chip', `s-${(form.status || '').toLowerCase()}`]"><i class="chip-dot"></i>{{ form.status
            }}</span>
        </div>
      ...
      <div v-if="showFormModal" class="ov" @click.self="closeFormModal">
        <div class="modal-xl">
          ...
          <div class="dh">
            <div class="dh-info">
              <div class="dh-badges">
                <span :class="['type-pill', activeForm?.type === 'IPCRF' ? 'pill-ipcrf' : 'pill-ccef']">{{
                  activeForm?.type }}</span>
                <span :class="['status-chip', `s-${(activeForm?.status || '').toLowerCase()}`]"><i class="chip-dot"></i>{{
                  activeForm?.status }}</span>
              </div>
              <div class="dh-name">{{ activeForm?.employeeName }}</div>
              <div class="dh-sub">Sem {{ activeForm?.semester }} · {{ activeForm?.year }} · {{ activeForm?.divisionName
                }}</div>
            </div>
            ...
            <div class="fn-section">
              <div class="fn-hd">
                <div class="fn-hd-l">
                  ...
                  <span class="fn-cnt">{{ coreEntries.length }} indicator{{ coreEntries.length !== 1 ? 's' : ''
                    }}</span>
                </div>
                <div class="fn-hd-r">
              ...
              <div v-else class="ind-list">
                <div v-for="e in coreEntries" :key="e.id" class="ind-card">
                  <div class="ind-card-hd">
                    ...
                    <div class="ind-acts">
                      <button class="act" @click.stop="openEditEntry(e)" title="Edit"><svg width="12" height="12"
                          viewBox="0 0 12 12" fill="none">
                          <path d="M8.5 1.5l2 2L4 10H2V8l6.5-6.5z" stroke="currentColor" stroke-width="1.2"
                            stroke-linejoin="round" />
                        </svg></button>
                      <button class="act act-del" @click.stop="askDelete(e)" title="Remove"><svg width="12" height="12"
                          viewBox="0 0 12 12" fill="none">
                          <path d="M2 3h8M5 3V2h2v1M3.5 3v6.5c0 .28.22.5.5.5h4c.28 0 .5-.22.5-.5V3"
            ...
            <div class="fn-section">
              <div class="fn-hd">
                <div class="fn-hd-l">
                  ...
                  <span class="fn-cnt">{{ supportEntries.length }} indicator{{ supportEntries.length !== 1 ? 's' : ''
                    }}</span>
                </div>
                <div class="fn-hd-r">
          ...
          <div v-else-if="activeTab === 'details'" class="modal-body-scroll">
            <div class="det-2col">
              <div>
                ...
                <div class="det-row"><span class="dk">Form Type</span><span class="dv">{{ activeForm?.type }}</span>
                </div>
                <div class="det-row"><span class="dk">Semester / Year</span><span class="dv">Sem {{ activeForm?.semester
                    }}, {{ activeForm?.year }}</span></div>
                <div class="det-row"><span class="dk">Position Level</span><span class="dv">{{ activeForm?.positionLevel
                    || '—' }}</span></div>
                <div class="det-row"><span class="dk">Division</span><span class="dv">{{ activeForm?.divisionName || '—'
                    }}</span></div>
                <div class="det-st" style="margin-top:16px">Timeline</div>
                <div class="det-row"><span class="dk">Created</span><span class="dv">{{ fmtDate(activeForm?.createdAt)
                    || '—' }}</span></div>
                <div class="det-row"><span class="dk">Submitted</span><span class="dv">{{
                  fmtDate(activeForm?.submittedAt) || '—' }}</span></div>
                <div class="det-row"><span class="dk">Approved</span><span class="dv">{{ fmtDate(activeForm?.approvedAt)
                    || '—' }}</span></div>
              </div>
              <div>
                <div class="det-st">Signatories</div>
                <div class="det-row"><span class="dk">Supervisor</span><span class="dv">{{
                  activeForm?.immediateSupervisor || '—' }}</span></div>
                <div class="det-row"><span class="dk">Supervisor Position</span><span class="dv">{{
                  activeForm?.supervisorPosition || '—' }}</span></div>
                <div class="det-row"><span class="dk">Approving Authority</span><span class="dv">{{
                  activeForm?.approvingAuthority || '—' }}</span></div>
                <div class="det-row"><span class="dk">Authority Position</span><span class="dv">{{
                  activeForm?.authorityPosition || '—' }}</span></div>
                <div class="det-st" style="margin-top:16px">Weights</div>
                <div class="weights-bar">
                  <div class="wb-c" :style="{ width: activeForm?.coreFunctionWeight + '%' }">Core {{
                    activeForm?.coreFunctionWeight }}%</div>
                  <div class="wb-s" :style="{ width: activeForm?.supportFunctionWeight + '%' }">Support {{
                    activeForm?.supportFunctionWeight }}%</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'score'" class="modal-body-scroll">
            ...
            <div v-else class="score-view">
              <div :class="['score-hero', scoreColorClass]"><span class="score-big">{{ activeForm?.finalNumericalRating
                  }}</span><span class="score-denom">/ 5.0</span></div>
              ...
              <div class="score-table">
                ...
                <div v-for="e in allEntries" :key="e.id" class="st-row">
                  <div class="st-l"><span :class="['st-fn', e.functionType === 'Core' ? 'fn-c' : 'fn-s']">{{ e.functionType[0]
                      }}</span><span class="st-name">{{ e.kraName }}</span></div>
        ...
        <div class="modal modal-lib">
          <div class="mhd">
            <div>
              <div class="mtitle">
                KRA Library
                <span :class="['type-pill ml6', currentFnType === 'Core' ? 'pill-ipcrf' : 'pill-ccef']"
                  style="font-size:10px">{{ currentFnType }}</span>
              </div>
              <div class="msub">Tap to select multiple indicators, then click <strong>Review Selection</strong></div>
            </div>
            <button class="xbtn" @click="cancelLibrary"><svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M1 1l12 12M13 1L1 13" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" />
              </svg></button>
          </div>
          ...
          <div class="mbody lib-scroll">
            ...
            <div v-else-if="!filteredLibrary.length" class="state-wrap">
              <p class="muted" style="font-size:12px">No matching indicators</p>
            </div>
            <div v-else class="lib-list">
              <div v-for="item in filteredLibrary" :key="item.id" :class="['lib-item', isSelected(item) && 'lib-sel']"
                @click="toggleSelect(item)">
                <div :class="['chk', isSelected(item) && 'chk-on']">
                  <svg v-if="isSelected(item)" width="9" height="9" viewBox="0 0 9 9" fill="none">
                    <path d="M1.5 4.5l2 2L7.5 2" stroke="#fff" stroke-width="1.6" stroke-linecap="round"
                      stroke-linejoin="round" />
                  </svg>
                </div>
                <div class="lib-content">
                  <div class="lib-kra-row">
                    ...
                    <div class="lib-tags">
                      ...
                      <span :class="['etag', item.classification !== 'Simple' ? 'etag-blue' : '']">{{ item.classification
                        }}</span>
                    </div>
                  </div>
                  <!-- Full text — readable -->
          ...
          <div class="mft lib-ft">
            <span class="muted" style="font-size:11px">{{ filteredLibrary.length }} result{{
              filteredLibrary.length !== 1 ?'s':'' }}</span>
            <div style="display:flex;gap:8px">
              ...
              <div class="ci-body">
                <div class="ci-header">
                  ...
                  <div class="ci-tags">
                    ...
                    <span :class="['type-pill', currentFnType === 'Core' ? 'pill-ipcrf' : 'pill-ccef']"
                      style="font-size:9px;padding:1px 6px">{{ currentFnType }}</span>
                  </div>
                </div>
        ...
        <div class="lock-box">
          ...
          <div class="lock-items">
            <div v-for="(item, i) in addProg.items" :key="i"
              ...
              <span
                :class="['lock-item-icon', addProg.current > i ? 'icon-done' : addProg.current === i ? 'icon-active' : 'icon-wait']">
          ...
          <div class="prog-track">
            <div class="prog-fill" :style="{ width: addProg.pct + '%' }"></div>
          </div>
    ...
    <teleport to="body">
      <div v-if="showNewFormModal" class="ov" @click.self="showNewFormModal = false">
        <div class="modal" style="max-width:480px">
          <div class="mhd">
            <div>
              <div class="mtitle">New Performance Form</div>
              <div class="msub">IPCRF or CCEF for this semester</div>
            </div><button class="xbtn" @click="showNewFormModal = false"><svg width="14" height="14" viewBox="0 0 14 14"
                fill="none">
                <path d="M1 1l12 12M13 1L1 13" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" />
              </svg></button>
          </div>
          <div class="mbody">
            ...
            <div class="fgrid">
              <div class="field"><label class="fl">Form Type</label><select v-model="newForm.type" class="fi">
                  <option>IPCRF</option>
                  <option>CCEF</option>
                </select></div>
              <div class="field"><label class="fl">Semester</label><select v-model="newForm.semester" class="fi">
                  <option value="1">1st Semester</option>
                  <option value="2">2nd Semester</option>
                </select></div>
              <div class="field"><label class="fl">Year</label><input v-model.number="newForm.year" type="number"
                  class="fi" /></div>
            </div>
            <div class="fsec" style="margin-top:16px">Signatories</div>
            <div class="fgrid">
              <div class="field full"><label class="fl">Immediate Supervisor</label><input
                  v-model="newForm.immediateSupervisor" type="text" class="fi" placeholder="Full name" /></div>
              <div class="field full"><label class="fl">Supervisor Position</label><input
                  v-model="newForm.supervisorPosition" type="text" class="fi"
                  placeholder="e.g. Division Chief / SWO V" /></div>
              <div class="field full"><label class="fl">Approving Authority</label><input
                  v-model="newForm.approvingAuthority" type="text" class="fi" placeholder="e.g. Helen Y. Suzara" />
              </div>
              <div class="field full"><label class="fl">Authority Position</label><input
                  v-model="newForm.authorityPosition" type="text" class="fi" placeholder="e.g. Bureau Director" /></div>
            </div>
          </div>
          <div class="mft"><button class="btn" @click="showNewFormModal = false">Cancel</button><button
              class="btn btn-primary" :disabled="creating" @click="createForm">{{ creating ? 'Creating…' : 'Create Form'
              }}</button></div>
        </div>
      </div>
    </teleport>
        ...
        <div class="modal" style="max-width:480px">
          <div class="mhd">
            <div>
              ...
              <div class="msub"><span :class="['type-pill', currentFnType === 'Core' ? 'pill-ipcrf' : 'pill-ccef']"
                  style="font-size:10px">{{ currentFnType }}</span></div>
            </div>
            <button class="xbtn" @click="closeEntry"><svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M1 1l12 12M13 1L1 13" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" />
              </svg></button>
          </div>
          <div class="mbody">
            <div class="fgrid">
              <div class="field full"><label class="fl">KRA Name <span class="req">*</span></label><input
                  v-model="entryForm.kraName" type="text" class="fi" placeholder="e.g. Research" /></div>
              <div class="field full"><label class="fl">Success Indicator <span class="req">*</span></label><textarea
                  v-model="entryForm.successIndicator" class="fi" rows="3"
                  placeholder="Describe the specific target output…"></textarea></div>
              <div class="field"><label class="fl">Weight (%)</label><input v-model.number="entryForm.weight"
                  type="number" class="fi" min="0" max="100" /></div>
              <div class="field"><label class="fl">Classification</label><select v-model="entryForm.classification"
                  class="fi">
              ...
              <div class="field full"><label class="fl">Applicable Period</label><select
                  v-model="entryForm.applicableRatingPeriod" class="fi">
                  <option>Both semesters</option>
                  <option>1st Semester</option>
                  <option>2nd Semester</option>
                </select></div>
              <div class="field full"><label class="fl">Means of Verification</label><input
                  v-model="entryForm.meansOfVerification" type="text" class="fi"
                  placeholder="e.g. Approved report with memo endorsement" /></div>
            </div>
            <template v-if="editingEntry">
              <div class="fsec" style="margin-top:16px">Rating</div>
              <div class="fgrid">
                <div class="field full"><label class="fl">Accomplishment</label><textarea
                    v-model="entryForm.accomplishment" class="fi" rows="2"></textarea></div>
                <div class="field"><label class="fl">Efficiency <span class="muted">(1–5)</span></label><input
                    v-model.number="entryForm.ratingEfficiency" type="number" class="fi" min="1" max="5" step="0.01" />
                </div>
                <div class="field"><label class="fl">Quality <span class="muted">(1–5)</span></label><input
                    v-model.number="entryForm.ratingQuality" type="number" class="fi" min="1" max="5" step="0.01" />
                </div>
                <div class="field"><label class="fl">Timeliness <span class="muted">(1–5)</span></label><input
                    v-model.number="entryForm.ratingTimeliness" type="number" class="fi" min="1" max="5" step="0.01" />
                </div>
                <div v-if="computedAvg" class="field"><label class="fl">Average</label>
                  <div class="fi avg-box">{{ computedAvg }}</div>
                </div>
              </div>
            </template>
          </div>
          <div class="mft"><button class="btn" @click="closeEntry">Cancel</button><button class="btn btn-primary"
              :disabled="savingEntry" @click="saveEntry">{{ savingEntry ? 'Saving…' : (editingEntry ? 'Save Changes' : 'Add Indicator')
              }}</button></div>
        </div>
      </div>
    </teleport>
    ...
    <teleport to="body">
      <div v-if="confirmDel.show" class="ov">
        <div class="confirm-box">
          <div class="cb-icon"><svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path d="M3 6h16M8 6V4h6v2M5 6v13a2 2 0 002 2h8a2 2 0 002-2V6" stroke="#ef4444" stroke-width="1.5"
                stroke-linecap="round" stroke-linejoin="round" />
            </svg></div>
          <div class="cb-title">Remove Indicator?</div>
          <div class="cb-msg">Remove <strong>{{ confirmDel.name }}</strong>?<br><span class="muted"
              style="font-size:11px">This cannot be undone.</span></div>
          <div class="cb-btns"><button class="btn" @click="confirmDel.show = false">Cancel</button><button
              class="btn btn-danger" :disabled="deletingEntry" @click="doDelete">{{ deletingEntry ? 'Removing…' : 'Remove'
              }}</button></div>
        </div>
      </div>
    </teleport>
